...
const toTitleCase = (arr) => {
  // Solution code here...
  return arr.map(str=>str[0].toUpperCase() + str.substring(1,str.length));
};
...
let biggerThanLuke = (arr) => {
  ...
  for (let i = 1;i < arr.length; i++){
    if(parseInt(arr[i].mass) > (arr[0].mass)){
      bigFellow += ' - ' + arr[i].name;
    }
  }
  return bigFellow.substring(3, bigFellow.length);
};
