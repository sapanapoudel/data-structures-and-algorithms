...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  return arr.map(e=>e.replace(/[()\s-]/g,''));
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  return str.split('').filter((el,i)=>i%2 !==0).join('');
};
...
const allHappy = (arr) => {
  // Solution code here...
  return arr.every(el => el.includes(':)'));
};
...
const findEvery = (arr, target) => {
  // Solution code here...
  return arr.every(el => el.includes(target));
};
...
const unenrollBrook = (arr) => {
  ...
  let resultArr=[];
  arr.map(el=>{
    resultArr.push((el.filter(str => !str.includes('Brook'))));
  });
  return resultArr;
};
