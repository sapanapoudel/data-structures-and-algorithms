...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  // Solution code here...
  const new_arr = arr.filter(b => {
    return (b.baseStat > minBaseStat);
  });
  return new_arr;
};
...
const getStatName = (arr, minBaseStat) => {
  // Solution code here...
  arr = arr.filter(b => b.baseStat > minBaseStat);
  arr = arr.map(b => b.stat.name);
  return arr;
};
...
const getCharactersWithoutChildren = (arr) => {
  // Solution code here...
  arr = arr.filter(characters => !(characters.children));
  return arr;
};
...
const evenOddNumericValues = (arr) => {
  // Solution code here...
  const new_arr = arr.filter(a => !(isNaN(a)));
  return new_arr.map(a => {
    if(a % 2 === 0){
      return a = 'even';
    }
    if(a % 2 !== 0) {
